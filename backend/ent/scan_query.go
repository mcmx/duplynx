...
type ScanQuery struct {
	...
	withInitiatedMachine *MachineQuery
...
func (_q *ScanQuery) QueryTenant() *TenantQuery {
...
// QueryInitiatedMachine chains the current query on the "initiated_machine" edge.
func (_q *ScanQuery) QueryInitiatedMachine() *MachineQuery {
...
func (_q *ScanQuery) QueryDuplicateGroups() *DuplicateGroupQuery {
...
func (_q *ScanQuery) FirstID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (_q *ScanQuery) FirstIDX(ctx context.Context) uuid.UUID {
...
func (_q *ScanQuery) OnlyID(ctx context.Context) (id uuid.UUID, err error) {
	var ids []uuid.UUID
...
func (_q *ScanQuery) OnlyIDX(ctx context.Context) uuid.UUID {
...
func (_q *ScanQuery) IDs(ctx context.Context) (ids []uuid.UUID, err error) {
...
func (_q *ScanQuery) IDsX(ctx context.Context) []uuid.UUID {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *ScanQuery) Clone() *ScanQuery {
	...
	return &ScanQuery{
		...
		withInitiatedMachine: _q.withInitiatedMachine.Clone(),
...
func (_q *ScanQuery) WithTenant(opts ...func(*TenantQuery)) *ScanQuery {
	query := (&TenantClient{config: _q.config}).Query()
...
// WithInitiatedMachine tells the query-builder to eager-load the nodes that are connected to
// the "initiated_machine" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *ScanQuery) WithInitiatedMachine(opts ...func(*MachineQuery)) *ScanQuery {
	query := (&MachineClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withInitiatedMachine = query
	return _q
}
...
func (_q *ScanQuery) WithDuplicateGroups(opts ...func(*DuplicateGroupQuery)) *ScanQuery {
	query := (&DuplicateGroupClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ScanQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Scan, error) {
	...
	if query := _q.withTenant; query != nil {
		if err := _q.loadTenant(ctx, query, nodes, nil,
			func(n *Scan, e *Tenant) { n.Edges.Tenant = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withInitiatedMachine; query != nil {
		if err := _q.loadInitiatedMachine(ctx, query, nodes, nil,
			func(n *Scan, e *Machine) { n.Edges.InitiatedMachine = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withDuplicateGroups; query != nil {
		if err := _q.loadDuplicateGroups(ctx, query, nodes,
			func(n *Scan) { n.Edges.DuplicateGroups = []*DuplicateGroup{} },
			func(n *Scan, e *DuplicateGroup) { n.Edges.DuplicateGroups = append(n.Edges.DuplicateGroups, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *ScanQuery) loadTenant(ctx context.Context, query *TenantQuery, nodes []*Scan, init func(*Scan), assign func(*Scan, *Tenant)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Scan)
	for i := range nodes {
		fk := nodes[i].TenantID
...
func (_q *ScanQuery) loadInitiatedMachine(ctx context.Context, query *MachineQuery, nodes []*Scan, init func(*Scan), assign func(*Scan, *Machine)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Scan)
	for i := range nodes {
		fk := nodes[i].InitiatedMachineID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "initiated_machine_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *ScanQuery) loadDuplicateGroups(ctx context.Context, query *DuplicateGroupQuery, nodes []*Scan, init func(*Scan), assign func(*Scan, *DuplicateGroup)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(duplicategroup.FieldScanID)
	}
	query.Where(predicate.DuplicateGroup(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(scan.DuplicateGroupsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *ScanQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(scan.Table, scan.Columns, sqlgraph.NewFieldSpec(scan.FieldID, field.TypeUUID))
	_spec.From = _q.sql
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withTenant != nil {
			_spec.Node.AddColumnOnce(scan.FieldTenantID)
		}
		if _q.withInitiatedMachine != nil {
			_spec.Node.AddColumnOnce(scan.FieldInitiatedMachineID)
		}
	}
